...
//constexpr int SOFTMAX_POLICY = 1;
...
protected:
    ...
    double m_epsilon;    // This is epsilon now
    double m_beta;
...
public:
    ...
    /**
     * Get an action using the learned Q-values (exploitation)
     * @param st the input state
     * @param ac the output action
     * @param relevants the set of relevant actions
     */
    void getLearnedAction(State& st, Action& ac, vector<int> relevants)
    {
        vect.clear();
        ...
        switch(Config::instance()->root["Learn"]["AfterLearn"]["PolicyFollowed"].asInt())
        {
