 ...
 */
bool SarsaAlgo::InitializeLearner(const string qFileName)
{
...
bool SarsaAlgo::threadInit()
{
    ...
    //Set initial states
    //Since now I am on the real robot, the initial state is not chosen randomly anymore but it is passed by the
    //"initial" argument
    resetStates.push_back(State(initial));
    relevantIndex = -1;
    for(int j=0; j<Config::instance()->root["Domain"]["Initial"].size(); j++){
        bool same = true;
        for(int i=0; i<Config::instance()->root["Domain"]["Initial"][to_string(j)].size(); i++){
            same &= (initial[i] == Config::instance()->root["Domain"]["Initial"][to_string(j)][i].asInt());
        }
        if(same) relevantIndex = j;
    }

    if(relevantIndex==-1){
        std::cout << "Initial state not supported" << endl;
        return false;
    }

    thisState = resetStates[0];
    ...
    thisAction = Action(int(uniformSampler.sample() * (thisAction.m_actions)));
 ...
 */
void SarsaAlgo::GetAction(State& s, Action& a)
{
 ...
 */
void SarsaAlgo::GetReward(State& st)
{
 ...
 */
bool SarsaAlgo::DoAction(State& st, Action& act)
{
    State tmp;
    act.apply(st,tmp);
